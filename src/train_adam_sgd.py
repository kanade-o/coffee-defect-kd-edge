...
#        tr_loss, tr_acc = run_epoch(model, train_dl, loss_fn, opt)
        ...
        # 2) 「訓練精度 95%」で Adam→SGD に切り替え
        if (tr_acc >= 0.95) and (not switched):
            logger.info(f"Train acc reached {tr_acc:.3%}, switching optimizer to SGD!")
            print(f"*** Switching optimizer to SGD (lr=0.01, momentum=0.002) ***")
            opt = torch.optim.SGD(
                model.parameters(),
                lr=0.01,
                momentum=0.002
