...
def attention_map(fm: torch.Tensor, eps=1e-8):
    # fm: (N, C, H, W) / ReLU後で合わせる（CNN224はforwardでF.reluを使うのでフックはpre-activationになるため）
...
def at_loss(feat_s, feat_t):
    # どちらも2D特徴に整形（teacher基準）し、AMをMSEで合わせる
    if feat_s.dim() == 3:  # (N, L, C) -> (N, C, H, W)
        feat_s = spatialize_tokens(feat_s)
    if feat_t.dim() == 3:
        feat_t = spatialize_tokens(feat_t)
...
def spatialize_tokens(x: torch.Tensor, grid_hw=None):
    ...
    if grid_hw is None:
        s = int(math.sqrt(L))
        if s * s != L:
            raise ValueError(f"Token length {L} is not square; set kd.teacher_grid_hw in config")
        H = W = s
    else:
        H, W = grid_hw
        assert H * W == L
    ...
    with torch.set_grad_enabled(train_mode):
        for x, y in loader:
            ...
            if f_s is None or f_t is None:
                raise RuntimeError("Feature hook returned None. Check kd.student_feat / kd.teacher_feat names.")

            # ViT → 2D 変換などは at_loss 内で吸収
...
@hydra.main(version_base=None, config_path="../configs", config_name="config_kd_cnn224")
def main(cfg: DictConfig):
    ...
    # ローカル重みの読み込み
    if getattr(cfg, "student_weights_path", None):
    ...
    # ---------- Hooks ----------
    # CNN224 向け既定：student_feat="conv3"（最終畳み込みの出力を使う）
    # teacher は最終畳み込み相当/ブロック終端など解像度の低い特徴を推奨
    s_feat_name = getattr(cfg.kd, "student_feat", "conv3")
    t_feat_name = cfg.kd.teacher_feat
    t_mod = find_module(teacher, t_feat_name)
    s_mod = find_module(student, s_feat_name)
